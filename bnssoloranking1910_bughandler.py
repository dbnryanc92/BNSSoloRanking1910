...
# Desc: Handle display bug in official ranking website on 191107
...
for i in range(headerrow + 1, len(tr_elements)):
    ...
    if len(row) != 5:
        break
    # Start of bug fixing part
    timestamp = row[4].text_content()
    timestamp = int(timestamp[5:7] + timestamp[8:10] + timestamp[11:13])
    if(timestamp < 110609):
        continue
